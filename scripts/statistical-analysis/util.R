...
readCsv <- function(file_name, getReal=TRUE, getArtificial=TRUE) {
    ...
    sloc <- fread("sloc.csv")
    #sloc <- fread("/home/nicolas/GitRepo/jaguar-data-flow-experiments/scripts/statistical-analysis/sloc.csv")
    ...
    data$KillDefn  <- "none"
    data$AggregationDefn  <- "none"
    data$KillDefnMacro  <- "none"
    data$AggregationDefn  <- "none"
    ...
    ochiai2             <- data$Family=="sbfl" & data$Formula=="ochiai2"             & data$TotalDefn=="tests"
    opt1                <- data$Family=="sbfl" & data$Formula=="opt1"                & data$TotalDefn=="tests"
    gp02                <- data$Family=="sbfl" & data$Formula=="gp02"                & data$TotalDefn=="tests"
    gp03                <- data$Family=="sbfl" & data$Formula=="gp03"                & data$TotalDefn=="tests"
    gp13                <- data$Family=="sbfl" & data$Formula=="gp13"                & data$TotalDefn=="tests"
    gp19                <- data$Family=="sbfl" & data$Formula=="gp19"                & data$TotalDefn=="tests"
    anderberg           <- data$Family=="sbfl" & data$Formula=="anderberg"           & data$TotalDefn=="tests"
    dice                <- data$Family=="sbfl" & data$Formula=="dice"                & data$TotalDefn=="tests"
    sorensen_dice       <- data$Family=="sbfl" & data$Formula=="sorensen_dice"       & data$TotalDefn=="tests"
    goodman             <- data$Family=="sbfl" & data$Formula=="goodman"             & data$TotalDefn=="tests"
    qe                  <- data$Family=="sbfl" & data$Formula=="qe"                  & data$TotalDefn=="tests"
    cbi_inc             <- data$Family=="sbfl" & data$Formula=="cbi_inc"             & data$TotalDefn=="tests"
    cbi_sqrt            <- data$Family=="sbfl" & data$Formula=="cbi_sqrt"            & data$TotalDefn=="tests"
    cbi_log             <- data$Family=="sbfl" & data$Formula=="cbi_log"             & data$TotalDefn=="tests"
    wong1               <- data$Family=="sbfl" & data$Formula=="wong1"               & data$TotalDefn=="tests"
    wong2               <- data$Family=="sbfl" & data$Formula=="wong2"               & data$TotalDefn=="tests"
    wong3               <- data$Family=="sbfl" & data$Formula=="wong3"               & data$TotalDefn=="tests"
    hamann              <- data$Family=="sbfl" & data$Formula=="hamann"              & data$TotalDefn=="tests"
    simple_matching     <- data$Family=="sbfl" & data$Formula=="simple_matching"     & data$TotalDefn=="tests"
    sokal               <- data$Family=="sbfl" & data$Formula=="sokal"               & data$TotalDefn=="tests"
    rogers_tanimoto     <- data$Family=="sbfl" & data$Formula=="rogers_tanimoto"     & data$TotalDefn=="tests"
    hamming             <- data$Family=="sbfl" & data$Formula=="hamming"             & data$TotalDefn=="tests"
    euclid              <- data$Family=="sbfl" & data$Formula=="euclid"              & data$TotalDefn=="tests"
    russell_rao         <- data$Family=="sbfl" & data$Formula=="russell_rao"         & data$TotalDefn=="tests"
    binary              <- data$Family=="sbfl" & data$Formula=="binary"              & data$TotalDefn=="tests"
    scott               <- data$Family=="sbfl" & data$Formula=="scott"               & data$TotalDefn=="tests"
    rogot1              <- data$Family=="sbfl" & data$Formula=="rogot1"              & data$TotalDefn=="tests"
    rogot2              <- data$Family=="sbfl" & data$Formula=="rogot2"              & data$TotalDefn=="tests"
    kulczynski1         <- data$Family=="sbfl" & data$Formula=="kulczynski1"         & data$TotalDefn=="tests"
    kulczynski2         <- data$Family=="sbfl" & data$Formula=="kulczynski2"         & data$TotalDefn=="tests"
    m1                  <- data$Family=="sbfl" & data$Formula=="m1"                  & data$TotalDefn=="tests"
    m2                  <- data$Family=="sbfl" & data$Formula=="m2"                  & data$TotalDefn=="tests"
    ample               <- data$Family=="sbfl" & data$Formula=="ample"               & data$TotalDefn=="tests"
    ample2              <- data$Family=="sbfl" & data$Formula=="ample2"              & data$TotalDefn=="tests"
    arithmetic_mean     <- data$Family=="sbfl" & data$Formula=="arithmetic_mean"     & data$TotalDefn=="tests"
    geometric_mean      <- data$Family=="sbfl" & data$Formula=="geometric_mean"      & data$TotalDefn=="tests"
    harmonic_mean       <- data$Family=="sbfl" & data$Formula=="harmonic_mean"       & data$TotalDefn=="tests"
    cohen               <- data$Family=="sbfl" & data$Formula=="cohen"               & data$TotalDefn=="tests"
    fleiss              <- data$Family=="sbfl" & data$Formula=="fleiss"              & data$TotalDefn=="tests"
    braun_banquet       <- data$Family=="sbfl" & data$Formula=="braun_banquet"       & data$TotalDefn=="tests"
    mountford           <- data$Family=="sbfl" & data$Formula=="mountford"           & data$TotalDefn=="tests"
    gower               <- data$Family=="sbfl" & data$Formula=="gower"               & data$TotalDefn=="tests"
    michael             <- data$Family=="sbfl" & data$Formula=="michael"             & data$TotalDefn=="tests"
    pierce              <- data$Family=="sbfl" & data$Formula=="pierce"              & data$TotalDefn=="tests"
    baroni_urbani_buser <- data$Family=="sbfl" & data$Formula=="baroni_urbani_buser" & data$TotalDefn=="tests"
    zoltar              <- data$Family=="sbfl" & data$Formula=="zoltar"              & data$TotalDefn=="tests"
    overlap             <- data$Family=="sbfl" & data$Formula=="overlap"             & data$TotalDefn=="tests"
    ...
    data[ochiai2,]$Technique             <- "ochiai2"
    data[opt1,]$Technique                <- "opt1"
    data[gp02,]$Technique                <- "gp02"
    data[gp03,]$Technique                <- "gp03"
    data[gp13,]$Technique                <- "gp13"
    data[gp19,]$Technique                <- "gp19"
    data[anderberg,]$Technique           <- "anderberg"
    data[dice,]$Technique                <- "dice"
    data[sorensen_dice,]$Technique       <- "sorensen_dice"
    data[goodman,]$Technique             <- "goodman"
    data[qe,]$Technique                  <- "qe"
    data[cbi_inc,]$Technique             <- "cbi_inc"
    data[cbi_sqrt,]$Technique            <- "cbi_sqrt"
    data[cbi_log,]$Technique             <- "cbi_log"
    data[wong1,]$Technique               <- "wong1"
    data[wong2,]$Technique               <- "wong2"
    data[wong3,]$Technique               <- "wong3"
    data[hamann,]$Technique              <- "hamann"
    data[simple_matching,]$Technique     <- "simple_matching"
    data[sokal,]$Technique               <- "sokal"
    data[rogers_tanimoto,]$Technique     <- "rogers_tanimoto"
    data[hamming,]$Technique             <- "hamming"
    data[euclid,]$Technique              <- "euclid"
    data[russell_rao,]$Technique         <- "russell_rao"
    data[binary,]$Technique              <- "binary"
    data[scott,]$Technique               <- "scott"
    data[rogot1,]$Technique              <- "rogot1"
    data[rogot2,]$Technique              <- "rogot2"
    data[kulczynski1,]$Technique         <- "kulczynski1"
    data[kulczynski2,]$Technique         <- "kulczynski2"
    data[m1,]$Technique                  <- "m1"
    data[m2,]$Technique                  <- "m2"
    data[ample,]$Technique               <- "ample"
    data[ample2,]$Technique              <- "ample2"
    data[arithmetic_mean,]$Technique     <- "arithmetic_mean"
    data[geometric_mean,]$Technique      <- "geometric_mean"
    data[harmonic_mean,]$Technique       <- "harmonic_mean"
    data[cohen,]$Technique               <- "cohen"
    data[fleiss,]$Technique              <- "fleiss"
    data[braun_banquet,]$Technique       <- "braun_banquet"
    data[mountford,]$Technique           <- "mountford"
    data[gower,]$Technique               <- "gower"
    data[michael,]$Technique             <- "michael"
    data[pierce,]$Technique              <- "pierce"
    data[baroni_urbani_buser,]$Technique <- "baroni_urbani_buser"
    data[zoltar,]$Technique              <- "zoltar"
    data[overlap,]$Technique             <- "overlap"
...
formatRow <- function(row, col) {
    ...
    #bestSBFL <- c(FamilyMacro=" ", FormulaMacro=getFormulaMacro("dstar2"), TotalDefn="tests", AggregationDefn="none", HybridScheme="none")
